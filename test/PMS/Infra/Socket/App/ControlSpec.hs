...
module PMS.Infra.Socket.App.ControlSpec (spec) where
...
import qualified PMS.Infra.Socket.App.Control as SUT
import qualified PMS.Infra.Socket.DM.Type as SUT
    ...
    context "when echo command issued." $ do
      it "should call callback" $ \ctx -> do 
        ...
        let domDat = ctx^.domainDataSpecContext
            ...
            cmdQ   = domDat^.DM.socketQueueDomainData
            resQ   = domDat^.DM.responseQueueDomainData
            expect = "abc"
            jsonR  = def {DM._jsonrpcJsonRpcRequest = expect}
            argDat = DM.SocketEchoCommandData jsonR ""
            args   = DM.SocketEchoCommand argDat
